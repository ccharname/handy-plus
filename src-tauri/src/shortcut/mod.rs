...
//!
//! - `tauri`: Uses Tauri's built-in global-shortcut plugin (Linux only)
//! - `handy_keys`: Uses the handy-keys library (macOS, Windows)
...
use crate::settings::{
    self, AutoSubmitKey, ClipboardHandling, LLMPrompt, OverlayPosition, PasteMethod,
...
/// Which keyboard backend is active.  On Linux we always use Tauri's built-in
/// global-shortcut plugin; on macOS and Windows we use handy-keys.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[allow(dead_code)] // `Tauri` is only constructed on Linux; variants used in match arms
...
/// Initialize shortcuts using the platform-appropriate implementation.
pub fn init_shortcuts(app: &AppHandle) {
    match KeyboardImplementation::current() {
...
pub fn change_binding(
    ...
    if let Err(e) =
        validate_shortcut_for_implementation(&binding, KeyboardImplementation::current())
    {
...
/// Validate a shortcut for the current platform keyboard implementation

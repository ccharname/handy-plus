...
// HANDY+ TEMP: regenerated on next cargo build
/**
 * Return the current foreground application (Power Mode).
 */
async getForegroundApp() : Promise<ForegroundApp | null> {
    return await TAURI_INVOKE("get_foreground_app");
},
/**
 * Add a new empty app profile.
 */
async addAppProfile(name: string) : Promise<Result<AppProfile, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("add_app_profile", { name }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
/**
 * Duplicate an app profile by id.
 */
async duplicateAppProfile(profileId: string) : Promise<Result<AppProfile, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("duplicate_app_profile", { profileId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type AppSettings = { bindings: Partial<{ [key in string]: ShortcutBinding }>; push_to_talk: boolean; audio_feedback: boolean; audio_feedback_volume?: number; sound_theme?: SoundTheme; start_hidden?: boolean; autostart_enabled?: boolean; update_checks_enabled?: boolean; selected_model?: string; always_on_microphone?: boolean; selected_microphone?: string | null; clamshell_microphone?: string | null; selected_output_device?: string | null; translate_to_english?: boolean; selected_language?: string; overlay_position?: OverlayPosition; debug_mode?: boolean; log_level?: LogLevel; custom_words?: string[]; model_unload_timeout?: ModelUnloadTimeout; word_correction_threshold?: number; history_limit?: number; recording_retention_period?: RecordingRetentionPeriod; paste_method?: PasteMethod; clipboard_handling?: ClipboardHandling; auto_submit?: boolean; auto_submit_key?: AutoSubmitKey; post_process_provider_id?: string; post_process_providers?: PostProcessProvider[]; post_process_api_keys?: SecretMap; post_process_models?: Partial<{ [key in string]: string }>; post_process_prompts?: LLMPrompt[]; post_process_selected_prompt_id?: string | null; mute_while_recording?: boolean; append_trailing_space?: boolean; app_language?: string; show_tray_icon?: boolean; paste_delay_ms?: number; typing_tool?: TypingTool; external_script_path: string | null; custom_filler_words?: string[] | null; ort_accelerator?: OrtAcceleratorSetting; extra_recording_buffer_ms?: number; power_mode_enabled?: boolean; app_profiles?: AppProfile[]; apple_speech_require_on_device?: boolean; diary_dir?: string | null; diary_keywords?: string[]; post_process_chain?: string[] | null; hotwords_boost?: number; active_preset_id?: string | null; profile_hot_swap_engine?: boolean; migration_applied?: Partial<{ [key in string]: boolean }> }
export type AsrPreset = { id: string; name: string; description: string; icon: string; model_id: string; language: string; require_post_process_chain: string[] | null; require_apple_speech_on_device?: boolean | null; builtin?: boolean }
